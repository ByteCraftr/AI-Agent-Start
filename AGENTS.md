...
当 Codex 不确定用户真实意图时，先问 1 个简短澄清问题，再执行或写入。

当用户在学习过程中提出方案偏好、个人理解、学习流程调整或学习效果评估请求时，Codex 先给出客观评价：说明优点、风险、隐含成本，并列出 1-3 个成熟方案或行业常见做法供学习和对照。用户选择适合自己的方案后，再进入定稿或写入。

学习资料默认采用 `Inbox -> Curated` 思路：临时遇到的概念先进入收集区，经过理解、归类和确认后，再整理进稳定知识库。
...
5. 看代码前，先用一个真实业务案例追踪“业务预期 -> Agent 流程 -> 模块 -> 函数 -> 数据结构”。
6. 完成最小代码、文档或测试产物。
7. 费曼复述后，给用户一个代表性小编码题，让用户写一个小函数和 2 个测试样例，再由 Codex 做代码反馈。
8. 运行、检查或审查，完成验收。
9. 记录笔记、周复盘和必要的产品/架构影响。

详细流程见 [docs/learning-workflow.md](docs/learning-workflow.md)。

常用意图和触发词见 [docs/intent-recognition.md](docs/intent-recognition.md)。
...
- `docs/intent-recognition.md`：学习协作中的意图识别、触发词和响应模式。
...
每次结束时，Codex 应该给出一个清晰的下一步，例如：“下一次直接说：领取任务。”
